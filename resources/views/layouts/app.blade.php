...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-satoshi antialiased">
        <div class="h-screen bg-gray-100">
            ...
            <main class="w-full h-full">
                @if(session()->has('success'))
                    <div id="successAlert" class="bg-green-400 rounded-md w-fit absolute top-5 right-10 border border-green-700">
                        <p class="text-white px-6 py-2">{{ session()->pull('success') }}</p>
                    </div>
                @elseif(session()->has('error'))
                    <div id="alert" class="bg-red-500 rounded-md w-fit absolute top-5 right-10 border-2 border-red-700">
        ...
        <script>
            ...
            // window.setTimeout(document.getElementById('successAlert').style.display='none', 5000);
        </script>
    </body>
</html>
